...
# Search archive.org for a Musopen recording in priority order. The
# Musopen collection on archive.org is a curated dump of CC0 / public
# domain classical recordings, so anything we get from there is safe.
QUERIES=(
  'collection:(musopen) AND title:(air on the g string) AND mediatype:(audio)'
  'collection:(musopen) AND title:(sheep may safely graze) AND mediatype:(audio)'
  'collection:(musopen) AND title:(jesu joy of mans desiring) AND mediatype:(audio)'
  'collection:(musopen) AND title:(canon in d) AND mediatype:(audio)'
  'collection:(musopen) AND creator:(bach) AND mediatype:(audio)'
)
...
try:
  ...
  print(docs[0]["identifier"] if docs else "")
except Exception:
  print("")
') || true
  if [ -z "$item" ]; then
